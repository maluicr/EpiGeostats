...
#' @export
 ...
 #
 #' @param mapobj string, name of list, output of function `outraster()`
 #' @param mapvar character, the map to be plotted ("etype", "unc", "sim")
 #' @param simid numeric, for mapvar = "sim", is the simulation #number to be plotted; if no value is assigned, first simulation is assumed.
 #' @param legname character, legend title.
 #'
 #' @return `spmap()` is used to map spatially continuous maps representing the median e-type disease risk
 #' or spatial uncertainty (inter-quartile difference) generated by the block direct sequential simulation algorithm.

 #' @export
 spmap <- function(mapobj = maps, mapvar = c("etype","unc", "sim"), simid = 1, legname = "Incidence"){
   ...
   if(m == "sim"){
     ...
     px_in <- raster::as.data.frame(mapobj[[m]][simid],
                                    row.names = NULL,
                                    optional = FALSE,
                                    xy = TRUE,
                                    na.rm = FALSE)
