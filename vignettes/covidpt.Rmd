...
vignette: >
  %\VignetteIndexEntry{Geostatistical COVID‑19 infection risk maps with R}
...
```{r block-data, warning = F, message = F, eval = F}
...
```{r varmodel-str, eval = F}
str(vmod)
```

```{r varmodel-plot, eval = F}
...
```{r spplot etype, eval = F}
spplot(maps[["etype"]])
```

```{r spplot uncert, eval = F}
